...
function knownImageModelToOption(model: KnownImageModel): ImageModelOption {
  return {
    modelId: model.modelId,
  ...
  return useMemo(() => {
    ...
    if (geminiProvider) {
      ...
      const models = mergeImageModels(
        mergeImageModels(KNOWN_GEMINI_IMAGE_MODELS.map(knownImageModelToOption), geminiImageModels),
        manualModels
      )
      if (models.length > 0) {
        groups.push({
    ...
    if (xaiProvider) {
      const manualModels = (providerSettingsMap?.[xaiProvider.id]?.models || [])
        .filter((model) => model.type === 'image' || model.modelId.startsWith('grok-imagine-image'))
        .map(manualImageModelToOption)
      const models = mergeImageModels(
        mergeImageModels(KNOWN_XAI_IMAGE_MODELS.map(knownImageModelToOption), xaiImageModels),
        manualModels
      )
      if (models.length > 0) {
        groups.push({
          label: xaiProvider.name,
