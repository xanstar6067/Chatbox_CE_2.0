    ...
    persist(
      ...
      })),
      {
        ...
        migrate: (persisted: any, version) => {
          const hadWebSearchMode = persisted?.extension?.webSearch?.mode !== undefined
          ...
          // Preserve the previous global-provider behavior for existing users.
          // Fresh installs use the new automatic model-aware routing default.
          if (version < 5 && !hadWebSearchMode) {
            settings.extension.webSearch.mode = 'provider'
          }
