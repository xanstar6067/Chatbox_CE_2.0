...
describe('processStreamChunk', () => {
  ...
  it('normalizes model-provided URL sources into the existing web search result UI shape', async () => {
    const state = createInitialState(undefined, 'Google Search')
    const withText = await processStreamChunk(chunk('text-delta', { text: 'Grounded answer' }), state, callbacks)
    const result = await processStreamChunk(
      chunk('source', {
        sourceType: 'url',
        id: 'source-1',
    ...
    expect(result.state.contentParts[1]).toEqual({ type: 'text', text: 'Grounded answer' })
  })

  it('preserves normalized sources when a server-side search tool result arrives', async () => {
    const state = createInitialState(undefined, 'xAI Search')
    const call = await processStreamChunk(
      chunk('tool-call', {
        toolCallId: 'search-1',
        toolName: 'web_search',
        args: {},
    ...
    const source = await processStreamChunk(
      chunk('source', {
        sourceType: 'url',
        id: 'source-1',
        url: 'https://x.ai/news',
    ...
    const result = await processStreamChunk(
      chunk('tool-result', {
        ...
        providerExecuted: true,
        output: {
          action: { type: 'search', query: 'latest xAI news' },
          sources: [{ type: 'url', url: 'https://x.ai/news' }],
        },
      }),
      source.state,
      callbacks
    ...
    expect(result.state.contentParts[0]).toMatchObject({
      type: 'tool-call',
      state: 'result',
      args: { provider: 'xAI Search', query: 'latest xAI news' },
      providerExecuted: true,
      result: {
        searchResults: [
          {
