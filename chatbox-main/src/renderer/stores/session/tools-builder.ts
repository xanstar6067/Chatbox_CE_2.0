...
export interface BuildToolsResult {
  tools: ToolSet
  instructions: string
  activeWebSearchMode: 'disabled' | 'model' | 'provider'
...
export async function buildToolsForSession(
  ...
  let activeWebSearchMode: BuildToolsResult['activeWebSearchMode'] = 'disabled'
  let nativeWebSearchProvider: string | undefined

  if (webBrowsing) {
    const webSearchSettings = settingActions.getExtensionSettings().webSearch
    const preferredMode = webSearchSettings.mode ?? 'provider'
    const nativeWebSearch =
      preferredMode === 'auto'
        ? model.getNativeWebSearch({
            hasCustomTools: Object.keys(tools).length > 0,
          })
        : null

    if (nativeWebSearch) {
      activeWebSearchMode = 'model'
      nativeWebSearchProvider = nativeWebSearch.provider
      tools = { ...tools, ...nativeWebSearch.tools }
      instructions += getToolSetDescription({ includeParseLink: false })
    } else {
      activeWebSearchMode = 'provider'
      const webSupported = model.isSupportToolUse('web-browsing')
      if (webSupported) {
        const includeParseLinkTool = PROVIDERS_WITH_PARSE_LINK.has(webSearchSettings.provider)
        tools.web_search = webSearchTool
        // Validation (license/API key) remains at execution time.
  ...
  return { tools, instructions, activeWebSearchMode, nativeWebSearchProvider }
}
