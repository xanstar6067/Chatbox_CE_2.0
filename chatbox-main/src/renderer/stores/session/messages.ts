...
export async function submitNewUserMessage(
  ...
  try {
    ...
    const webSearchSettings = globalSettings.extension.webSearch
    const hasNativeWebSearch =
      webSearchSettings.mode === 'auto' && model.getNativeWebSearch({ hasCustomTools: false }) !== null
    if (webBrowsing && platform.type === 'web' && !model.isSupportToolUse() && !hasNativeWebSearch) {
