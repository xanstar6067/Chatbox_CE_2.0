...
describe('buildToolsForSession web search tools', () => {
  beforeEach(() => {
    mockSettings.mode = 'provider'
    mockSettings.provider = 'bing'
  })

  function makeModel(
    toolUseSupported = true,
    nativeWebSearch: ReturnType<ModelInterface['getNativeWebSearch']> = null
  ): ModelInterface {
    return {
      name: 'mock',
      modelId: 'mock-model',
      ...
      isSupportToolUse: () => toolUseSupported,
      isSupportSystemMessage: () => true,
      getNativeWebSearch: () => nativeWebSearch,
      chat: vi.fn(),
      chatStream: vi.fn(),
      paint: vi.fn(),
    } as unknown as ModelInterface
  ...
  it('uses the current model search in automatic mode when available', async () => {
    mockSettings.mode = 'auto'
    const nativeTool = { type: 'provider', id: 'google.google_search', args: {} }

    const result = await buildToolsForSession(
      makeModel(true, {
        provider: 'Google Search',
        tools: { web_search: nativeTool } as never,
      }),
      {
        webBrowsing: true,
        messages: [],
    ...
    expect(result.activeWebSearchMode).toBe('model')
    expect(result.nativeWebSearchProvider).toBe('Google Search')
    expect(result.tools.web_search).toBe(nativeTool)
    expect(result.tools).not.toHaveProperty('parse_link')
  })

  it('falls back to the configured provider when the model has no native search', async () => {
    mockSettings.mode = 'auto'
    ...
    expect(result.activeWebSearchMode).toBe('provider')
