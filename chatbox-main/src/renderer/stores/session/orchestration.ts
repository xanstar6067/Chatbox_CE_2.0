...
export async function orchestrateGeneration(
  ...
  try {
    ...
    const { tools, instructions, activeWebSearchMode, nativeWebSearchProvider } = await buildToolsForSession(model, {
      webBrowsing,
      knowledgeBase,
      messages: promptMsgs,
    })
    ...
    const { promptMsgs: updatedMsgs, fallbackToolCallPart } = await applyLegacyToolFallback({
      ...
      promptMsgs,
      knowledgeBase,
      webBrowsing: activeWebSearchMode === 'provider',
    ...
    const chatOptions: ChatStreamOptions = {
      ...
      webSearchMode: activeWebSearchMode === 'disabled' ? undefined : activeWebSearchMode,
    }
    ...
    let processorState = createInitialState(
      fallbackToolCallPart ? [fallbackToolCallPart] : undefined,
