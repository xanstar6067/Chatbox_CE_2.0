...
export async function modifyNameAndThreadName(sessionId: string, name: string) {
  await chatStore.updateSession(sessionId, (session) => {
    ...
    // A title request may finish after the user has renamed the conversation.
    // Only an untouched new conversation is eligible for an automatic title.
    if (session.name !== 'Untitled') {
      return session
    }
    return { ...session, name, threadName: name }
  })
}
