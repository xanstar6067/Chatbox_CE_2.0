...
describe('settingsStore persistence', () => {
  ...
  it('rehydrates persisted provider and custom provider settings', async () => {
    ...
    expect(settingsStore.getState().extension.webSearch.mode).toBe('provider')
  })
  ...
  it('uses automatic model-aware web search routing for fresh settings', async () => {
    const { initSettingsStore, settingsStore } = await loadSettingsStoreModule()

    await initSettingsStore()

    expect(settingsStore.getState().extension.webSearch.mode).toBe('auto')
  })
})
